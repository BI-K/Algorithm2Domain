...
Each create script run will create a new folder in the `outputs/create/` directory with the name of the current datetime. The folder will contain analysis, reports, logs, and mainly data folder which will have like folder names with the subject ID and the generated csv files using the records in the subject ID folder.

## 3) Split Database Script

- `split_dataset.py`: This script splits the MIMIC-III Waveform Database Matched Subset into train, validation, and test sets.

You can run it with the command:
```bash
# To run the split script
python scripts/split_dataset.py --config configs/split/default.json
```

The splitting dataset must split the dataset created by the `create_dataset.py` script. 
...
## 4) Transform data to required format that is accespted by Algorithm2Domain_ADATime
